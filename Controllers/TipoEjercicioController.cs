...
public class TipoEjerciciosController : Controller
{
    ...
    {
        var tipoEjercicio = _context.TipoEjercicios.Find(tipoEjercicioID);
        //Validacion eliminar, analizando si se encuentra en la otra tabla
        var ejerciciosFisicosEnEstados = _context.EjerciciosFisicos.Any(t => t.TipoEjercicioID == tipoEjercicioID);

        if (ejerciciosFisicosEnEstados)
        {
            return Json("");
        }
        _context.Remove(tipoEjercicio);
